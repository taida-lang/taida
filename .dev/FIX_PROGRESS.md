...
| ID | severity | 概要 | ファイル | 状態 |
|----|----------|------|---------|------|
| M-08 | minor | `taida_sha256` Bytes 分岐: 上限ガード追加 | `src/codegen/native_runtime.c` | `VERIFIED` |
| M-12 | minor | `taida_os_list_dir`: capacity オーバーフロー防御 | `src/codegen/native_runtime.c` | `VERIFIED` |
| M-14 | minor | NULL チェックなしの raw malloc を TAIDA_MALLOC に統一 | `src/codegen/native_runtime.c` | `VERIFIED` |
