...
CLI のコミュニティ機能を実際に使える状態にするリリースです。
`taida community` の認証は設計当初からサーバ側の対応を欠いており、
認証付きコマンドはすべて 401 になっていました（taida.dev backend の
Taida 移行で発見）。

### Added

- **`taida auth login` がブラウザを自動で開きます**: Device Flow の
  検証 URL をプラットフォームのオープナー（xdg-open / open /
  cmd start）で開き、TTY では URL を OSC 8 ハイパーリンクとして
  表示します。ヘッドレス環境では従来どおり URL の手動オープンに
  フォールバックします。コードの入力は引き続き手動です（GitHub の
  device エンドポイントにプリフィル機構がないため）。

### Fixed

- **`taida community post` / `message` 等の認証付きコマンドが
  機能していなかった問題**: login は GitHub のアクセストークンを
  保存し、community コマンドはそれを Bearer として送っていましたが、
  taida.dev が受けるのは自前のセッショントークンだけでした。login が
  taida.dev の `POST /auth/device` で GitHub トークンをセッション
  トークンに交換して保存するようになり、community コマンドは
  セッショントークンだけを送ります。GitHub トークンは従来どおり
  GitHub への呼び出し（`ingot install` / `publish`）専用です。
  交換ステップ導入前の `~/.taida/auth.json` はそのまま読め、401 の
  代わりに再ログインを促す明確な案内が出ます。
