...
- Enum 型は序数値を持つ
- インタプリタを基準に、ネイティブ + WASM プロファイルが同一出力を返す
  パリティ運用
...
正式バックエンドは、同一ソースから同一出力を返します。WASM プロファイルは `wasm-min` / `wasm-wasi` /
`wasm-edge` / `wasm-full` の 4 種で、各プロファイルの対応範囲は
[WASM プロファイル](docs/reference/wasm_profiles.md) を参照してください。
旧 JS ターゲットは移行期間の互換機能であり、リリースのパリティ契約には含みません。
...
| [メモリ管理モデル](docs/reference/memory_model.md) | バックエンド別のメモリ戦略とアドオン所有権規約 |
...
| [`taida-lang/js`](docs/api/js.md) | 旧 JS ターゲット向け相互運用の実行記述 API |
