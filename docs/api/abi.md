...
する場合、現行 MVP の低レベル ABI は allocator 領域を request ごとに巻き戻し
ません。長寿命 host は request ごとに instance を作り直すか、host 側で instance
lifetime を 1 request に限定してください。

WASM handler の `req.body` は `bytes(req.body)` で response body に転送する
用途を保証します。現行 MVP では WASM 上の `req.body.length()` や index access は
portable な契約ではありません。body length や byte 単位処理が必要な handler は、
Native backend を使うか、WASM Bytes runtime 統一後の backend を対象にしてください。
