...
上記以外の文脈でディスクリプタをランタイム値として使った場合
(`stdout(serverX)`、ユーザー関数引数、`Str[serverX]()` のような変換 /
モールド引数、演算オペランド、フィールド / メソッドアクセスなど) は、型
チェッカーが `[E1532]` で reject します。ディスクリプタはビルドドライバ
専用値であり、ランタイムロジックに混ぜることはできません。許可される
ディスクリプタの出現位置は本節冒頭のリスト (トップレベル export 値、別の
ディスクリプタのフィールド、export へ到達させるためのトップレベル変数束縛の
右辺) に限られます。

> ディスクリプタビルド経路 (`taida build --unit` / `--plan` / `--all-units`)
> はディスクリプタモジュールを直接解析・取り込みするため型チェッカーを
> 経由せず、`[E1532]` の対象外です。`[E1532]` はディスクリプタモジュールを
> `taida run` / `taida way check` / 単一ターゲットビルドに渡したとき、つまり
> ディスクリプタをランタイム値として扱おうとしたときにのみ発火します。
...
|-----------|-------------|--------|------|
| ディスクリプタ式の評価 (export 値 / ディスクリプタフィールド / 束縛 RHS) | 受理 | 受理 | 受理 |
| ランタイム値としての使用 | `[E1532]` で reject (§6 参照) | 同左 | 同左 |

「ランタイム値としての使用」の `[E1532]` は型チェッカーが発火する前段ゲート
であり、3 バックエンド共通です。ディスクリプタビルド経路
(`taida build --unit` / `--plan` / `--all-units`) は型チェッカーを経由しない
ため対象外です (§6 の注記を参照)。
