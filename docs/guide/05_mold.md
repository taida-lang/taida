...
ヘッダー (`Mold[T] => MyMold[T]`) は親型 arity 一致が要件です ([04_class_like.md](04_class_like.md#親型の型引数と-arity-一致))。`Mold[...]` の親側は常に 1 スロットのままで、追加スロットは子側 `MyMold[T, U]` 等で書きます。
...
// コンパイルエラー: 2 つ目のヘッダースロットに対応するフィールドが無い
```
...
| 形式 | 意味 | 例 |
|------|------|------|
| `Mold[:Int]` | 具体型 `Int` 専用の鋳型 | `Mold[:Int] => IntBox = @()` |
| `Mold[T]` | 任意の型を受ける鋳型 (型変数 `T`) | `Mold[T] => Box[T] = @()` |
| `Mold[T <= :Int]` | 具体型制約付き型変数 | `Mold[T <= :Int] => NumBox[T] = @()` |

**`:` を欠いた具体型** (`Mold[Int]`) は、組み込み型名 (`Int` / `Str` / `Bool` / `Float` / `Bytes` / `Lax` / `Result` / `Async` / `Optional` 等) と衝突する型変数名として書かれた場合、型チェッカーが `[E1523]` で reject します。`Mold[Int] => MyMold[Int] = @()` は文法上は通りますが、`Int` 型変数の意図か具体型 `Int` の意図か曖昧なため、明示的に `Mold[:Int]` (具体型直接指定) または `Mold[T <= :Int]` (制約付き型変数) のどちらかに書き換える必要があります。**具体型を使うときは必ず `:` を付ける** と覚えてください。
...
// コンパイルエラー: 具体型スロットもフィールドスロットを消費するため、U の束縛先が無い
...
| ケース | 振る舞い |
|---|---|
| 同名フィールド・同一型で再宣言 | idempotent (許可、最後の宣言が有効) |
...
`_` プレースホルダの全体仕様 (パイプライン内 / 関数引数の `f(a, , c)` カンマスキップ等) は [`docs/guide/00_overview.md`](00_overview.md) のプレースホルダ節を参照してください。
...
Cage は外部由来の Molten 値を扱うための **境界 (boundary)** です。`Cage[subject, runner]()` は subject の Molten branch と runner の `CageRilla[Branch, Out]` 子系統 descriptor の `Branch` を型レベルで照合し、一致したときだけ branch operation を実行して結果を `Gorillax[Out]` で返します。
...
CageRilla 子系統と JSRilla 系 constructor (`JSGet` / `JSCall` / `JSNew` / `JSSet` / `JSBind` / `JSSpread`) の詳細は [`docs/api/js.md`](../api/js.md) を参照してください。`JSRilla` 子系統は **JS バックエンド専用** で、インタプリタ・Native・WASM では利用できません。
...
| npm パッケージの Molten 操作 | `Cage[subject, JSRilla[...]()]()` | `Gorillax[Out]` |
...
| 継承オーバーライド | 同名同型 idempotent / 同名異型 `[E1411]` / `solidify` / `unmold` 上書き可 |
