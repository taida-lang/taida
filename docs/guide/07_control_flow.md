...
default arm (`| _ |>` または `| true |>`) は条件分岐に必須です。欠落した条件分岐は型チェッカーで `[E1524]` として reject されます。default arm を最後に置くと、上から順に評価する読み方がそのまま「いずれにも合致しなかったときの fallback」になり、読み手が混乱しません。実行は上から順なので、default arm を途中に置くと後続の arm には制御が到達しなくなることに注意してください。
...
パイプラインで `_` を使って前段の値を参照できます。`_` (注入位置) は 1 ステージに 1 個までです。前段の値を複数の位置で使う場合は、`=> v` の束縛転送 (bind-and-forward) で名前を付けてから参照します。

```taida
// 分類: `_` 1 個はそのまま注入
5 => If[_ > 3, "big", "small"]() => label  // "big"

// clamp: 値を 2 箇所で使うので束縛転送で名前を付ける
...
> 注: この短絡は `If` がコンパイラに特別扱いされる構文 (special form) であることによる性質です。`If` をユーザー関数でラップする (`myIf c: Bool  a: Int  b: Int = If[c, a, b]() => :Int`) と、関数呼び出しの引数は呼び出し前にすべて評価されるため、**呼び出し地点で両方の枝が評価され短絡は失われます**。枝の遅延評価が必要な場合はラップせず `If` を直接書いてください。
