...
`ErrorInfo` シェイプは `@(type: Str, message: Str, kind: Str, code: Int)`。成功値や詳細を持たない空の `Lax` では `errorInfo()` の返す `Lax` の `hasValue = false`、詳細を持つ失敗時のみ `Lax(true)` で `ErrorInfo` を取り出せます。`docs/reference/class_like_types.md` の「errorInfo() による失敗情報取得」「ErrorInfo」節を参照。
