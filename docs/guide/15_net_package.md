...
同梱パッケージなので `taida ingot install` は不要、`>>>` 1 行で全 API が
使えます。3 バックエンド（Interpreter / JS / Native）で挙動が一致し、
WASM バックエンドは現状 NET の API を提供しません（該当呼び出しは
`[E1612]` で拒否されます。各 WASM プロファイルの対応状況は
[`docs/reference/wasm_profiles.md`](../reference/wasm_profiles.md) を参照）。

詳細な API シグネチャは [`docs/api/net.md`](../api/net.md)
...
詳細は [`docs/api/net.md` §8](../api/net.md) の
...
runtime error になります (`docs/api/net.md` §8 の writer state
machine を参照)。
...
TLS 構成と h2 / h3 サポートの詳細は `docs/api/net.md` の Backend
scope 節を参照してください。
...
| Native | ○ | HTTP/1.1 + HTTP/2 (TLS 含む) + HTTP/3 の cdylib ランタイムを同梱 |
...
- `taida-lang/net` はコア同梱パッケージのため、`taida ingot install` も
  `packages.tdm` への宣言も不要で、`>>>` 1 行で読み込めます。
- 同名のユーザー作成パッケージで上書きすることはできません（リゾルバが
  同梱パッケージを優先します）。
- 全関数は `taida-lang/net` の `<<<` で固定エクスポートされており、
  ぶちパックフィールド名 / モールド名 / 関数名のいずれも変更できません
  （stable な公開 API 契約）。

詳細な API シグネチャ・error condition・implementation 参照は
[`docs/api/net.md`](../api/net.md) にあります。
公開仕様の保証範囲については
[`docs/reference/release_process.md`](../reference/release_process.md)
を参照してください。
