...
| `E1546` | モールド呼び出しの cast `()` 欠落。値位置 (代入 RHS / パイプ段 / 関数引数 / アンモールド source 等) の bare `Name[args]` は受理されない — `[]` は鋳型に入れる、`()` は鋳造する、の 2 段階であり cast は省略不可。型位置 (型注釈 `:Lax[Int]`、他モールドの `[...]` 引数内の型参照 `JSON[x, Schema]()` の `Schema` 等) は型参照なので `()` を付けない。 | Parser |
