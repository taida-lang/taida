...
| 4 | `>=>` | 右アンモールド | `solidify` 後の値から `unmold` で取り出して右側へ渡す（`=>` と同じ右方向） |
| 5 | `<=<` | 左アンモールド | `solidify` 後の値から `unmold` で取り出して左側へ渡す（`<=` と同じ左方向） |
...
4 つのバックエンド (Interpreter / JS / Native / WASM) で同一の結果を返します。床除算が必要な場合は、得られた商と剰余からユーザコード側で補正してください。
