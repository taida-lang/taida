...
> **注意**: 現在のパーサーは `<=` の単一代入のみをサポートします。`result <= f() <= g() <= data` のような `<=` のチェーン（逆方向パイプライン）は受理されません。逆方向のデータフローが必要な場合は、`=>` の順方向パイプラインまたは中間変数を使用してください。
...
> **注意**: `<=` のチェーン（`result <= f() <= g() <= data` のような2段以上の連鎖）はパーサーが現在受理しません。逆方向のデータフローが必要な場合は `=>` の順方向パイプラインまたは中間変数を使用してください。
