...
sumTail list: @[Int] acc: Int =
  | list.isEmpty() |> acc
  | _ |> sumTail(list.drop(1), acc + list.first())
=> :Int
```
...
reverseTail list: @[T] acc: @[T] =
  | list.isEmpty() |> acc
  | _ |> reverseTail(list.drop(1), acc.prepend(list.first()))
=> :@[T]
```
...
processItemsTail items: @[Item] acc: @[Result] =
  | items.isEmpty() |> acc
  | _ |>
    result <= processOne(items.first())
    processItemsTail(items.drop(1), acc.append(result))  // 末尾位置
=> :@[Result]
```
