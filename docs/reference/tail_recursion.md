...
| Native      | OK | 現時点では `[E0700]` で拒否 (整備中) |
| WASM (Native と同系統のコード生成を行うプロファイル) | OK | 現時点では `[E0700]` で拒否 (整備中) |

Native、および Native と同系統のコード生成を行う WASM プロファイルでは、相互再帰を現時点で `[E0700]` でコンパイル時に拒否します。これはトランポリンまたは等価機構の整備が進行中であるための過渡的な制約です。バックエンド間で動作が気付かれずに乖離するのを避けるため、実行時のスタックオーバーフローではなくコンパイル時の診断として明示しています。

正式バックエンドのパリティ達成 (相互再帰を Native / WASM でも実行可能にする) が目標です。Native / WASM 側のトランポリン整備が完了するまでは、`[E0700]` で拒否される入力は次のいずれかで回避します。

#### `[E0700]` が出たときの対応

1. 相互再帰を直接再帰に書き換える (アキュムレータ引数渡しや 1 関数への統合)
2. 上記が難しい場合、対象処理を Interpreter 実行に分離する
