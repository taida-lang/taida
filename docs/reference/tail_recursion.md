...
| WASM (Native と同系統の lowering を行うプロファイル) | OK | 現時点では `[E0700]` で拒否 (整備中) |

Native と Native と同系統の lowering を行う WASM プロファイルでは、相互再帰を現時点で `[E0700]` でコンパイル時に拒否します。これはトランポリンまたは等価機構の整備が進行中であるための過渡的な制約であり、バックエンド間で動作が暗黙のうちに乖離するのを避ける目的で、ランタイムでのスタックオーバーフローではなくコンパイル時の診断として明示しています。

4 バックエンドのパリティ達成 (相互再帰を全バックエンドで実行可能にする) が目標です。Native / WASM 側のトランポリン整備が完了するまでは、`[E0700]` で拒否される入力は次のいずれかで回避します。
