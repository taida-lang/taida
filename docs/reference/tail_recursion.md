...
- **非末尾自己再帰はコンパイルエラー**: 直接再帰 (`A → A`) の呼び出しが末尾位置にない場合、`[E0701]` でコンパイル時に reject されます。ランタイムのスタックオーバーフローを防ぐためのコンパイル時診断です
...
```taida fragment
// 再帰呼び出しの後に演算がある (`[E0701]` で reject される反例)
factorial n: Int =
  | n < 1 |> 1
...
process x: Int =
  |== error: Error =
    defaultValue  // ← ハンドラ末尾の式 (戻り値の決定)
  => :Int

  calculate(x)    // ← 関数本体末尾の式 = 末尾位置
=> :Int
```

ceiling ハンドラ内 (`|== error: Error = ...`) の最後の式はハンドラの戻り値を決定しますが、ハンドラは元の関数のトランポリンとは別スコープで実行されるため、**ハンドラ内に再帰呼び出しを置いた場合は末尾呼び出し最適化の対象になりません**。再帰呼び出しは ceiling の外側に置きます (「エラーシーリングとの相互作用」節を参照)。
...
```taida fragment
// NOT 末尾位置: 乗算の引数 — `[E0701]` で reject される反例
badFactorial n: Int =
  | n < 1 |> 1
...
```taida fragment
// NOT 末尾位置: wrapの引数 — `[E0701]` で reject される反例
foo x: Int =
...
`If[cond, then, else]()` は条件に応じて `then` または `else` 片方だけを評価する短絡評価ですが、**型引数として渡された再帰呼び出しは末尾位置として扱われません**。If モールド内に再帰呼び出しを書いた場合は、深さ n に応じて呼び出しスタックを消費します。

```taida fragment
// NOT 末尾位置: If モールドの型引数は末尾位置にならない
countdown n: Int =
  If[n == 0, 0, countdown(n - 1)]()  // × スタックが n 回積まれる
=> :Int
```

末尾再帰として動作させたい場合はガード式 (`| cond |>`) に書き換えます。ガード式の各アーム本体の末尾は末尾位置として扱われ、TCO の対象になります。

```taida
// 末尾位置: ガード式アーム本体は TCO の対象
countdown n: Int =
  | n == 0 |> 0
  | _ |> countdown(n - 1)  // 末尾位置
=> :Int
```
...
```taida fragment
// スタックが n 回積まれる (`[E0701]` で reject される反例)
factorial n: Int =
  | n < 1 |> 1
  | _ |> n * factorial(n - 1)
=> :Int
...
|-------------|:---------------------------:|:--------:|
| Interpreter | OK | OK (トランポリン) |
| Native      | OK | OK (末尾限定サイクルをディスパッチャに統合) |
| WASM (Native と同系統のコード生成を行うプロファイル) | OK | OK (同上) |

Native、および Native と同系統のコード生成を行う WASM プロファイルでは、末尾位置のみで構成される相互再帰サイクルを、コンパイル時に単一の自己末尾再帰関数（タグでメンバーを切り替えるディスパッチャ）へ統合して最適化します。元の関数は薄いラッパーとして保持されるため、外部からの呼び出し・export・値としての参照は変わりません。

ディスパッチャ統合の対象となるのは、サイクル内の相互呼び出しがすべて次を満たす場合です:

1. **単純な末尾位置**にあること — 関数本体の最終文、または条件分岐アームの最終文 (の連鎖)。パイプライン段・エラー天井ハンドラ・lambda 内・引数位置の呼び出しは対象外
2. **全引数を明示**して呼び出していること (デフォルト引数の補完に依存しない)
3. 呼び出しの引数内にサイクル内関数の呼び出しをネストしていないこと

この条件を外れる相互サイクルは、実行時スタックオーバーフローを避けるため引き続き `[E0700]` でコンパイル時に拒否されます。

### 非末尾再帰の深度上限

末尾位置にない再帰はスタックを消費します。深度超過はどのバックエンドでも**診断付きで停止**します（無言クラッシュしません）:

| バックエンド | 上限 | 失敗時の動作 |
|-------------|------|-------------|
| Interpreter | 呼び出し深度 8192 | `Maximum call depth (8192) exceeded` で停止 |
| Native | OS スタック残量ベース（通常は深度 8192 を大きく超える容量） | `Maximum call stack depth exceeded` を表示して exit 1 |
| WASM | ランタイムのスタック上限 | `call stack exhausted` トラップ |

Interpreter の上限が最も浅いため、Interpreter で動くプログラムは他バックエンドでも深度起因では失敗しません。深い処理は末尾再帰（深度消費ゼロ）に書き換えてください。

#### `[E0700]` が出たときの対応

1. サイクル内の相互呼び出しをすべて末尾位置に移し、引数を明示する (上記の統合条件に入れる)
2. 相互再帰を直接再帰に書き換える (アキュムレータ引数渡しや 1 関数への統合)
3. 通常のループに置き換える (`@[1..n].reduce(_)` など)

### 非末尾の相互再帰はコンパイルエラー

非末尾位置での相互再帰は、実行時に必ずスタックオーバーフロー（`Maximum call depth (8192) exceeded`）を起こすため、コンパイル時に拒否されます。
...
3. サイクルに含まれる 2 関数以上のノードごとに、**呼び出し元の AST を走査し末尾位置判定を行う**。末尾位置の規則は上記「末尾位置とは」と同一。
4. サイクル上のいずれかのエッジに**非末尾**呼び出しが存在すれば `[E1614]` を発行してコンパイルを停止する。
5. すべてのエッジが末尾位置であれば、検査はパスする（Interpreter ではトランポリン、Native では通常呼び出しで動作）。
...
エラーシーリングを伴う再帰関数は、**ハンドラブロックを関数本体の先頭に置き、再帰本体は ceiling の外** に置くのが標準パターンです。再帰呼び出し自体は ceiling の外側にあるため、関数本体の最後の式として通常どおり末尾位置になります。

```taida
// xs を末尾再帰で合計する。途中で processOne が throw した場合は
// エラーシーリングが 0 にフォールバックする。
safeSum xs: @[Int] =
  |== error: Error =
    0          // ceiling ハンドラの戻り値。再帰ループとは別経路。
  => :Int

  sumTail(xs, 0)  // ceiling の外、関数本体の最後の式 = 末尾位置
=> :Int
...
sumTail xs: @[Int] acc: Int =
  ...
  | _ |>
    xs.first() >=> head
    Drop[xs, 1]() >=> rest
    value: Int <= processOne(head)  // throw した場合は safeSum 側の ceiling が捕捉
...
- ceiling ハンドラ (`|== error: Error = ... => :Int`) は関数本体先頭で宣言し、ハンドラ本体は失敗時の戻り値の決定だけを行う。
- 再帰呼び出し (`sumTail(rest, acc + value)`) は ceiling の外側、`| _ |>` アーム本体の最後の式に置く。
- ハンドラ内部から再帰呼び出しを行わない (例: `|== error: Error = sumTail(...)`)。ハンドラ内の呼び出しは末尾呼び出し最適化の対象外として扱われるため、深い再帰でスタックを消費します。
...
- **コンパイラ警告**: 現状の実装は非末尾位置の再帰呼び出しを検出して警告しません。再帰関数を書く際は「チェックリスト」節を見て自分で確認してください
...
1. **再帰呼び出しは最後の操作か？**
   - 再帰呼び出しの後に演算がないこと
   - 再帰呼び出しが `If[...]()` モールド内に閉じ込められていないこと (If モールドの型引数は末尾位置にならない)
...
5. **深さが入力サイズに比例する再帰でないか？**
   - 直接再帰でも非末尾位置のままだと O(N) スタックを消費する
   - 大きな N を扱う再帰関数は必ずアキュムレータ + 末尾再帰に書き換える
