...
この差分は 4 バックエンド・パリティの例外として扱います。Interpreter / JS は相互再帰を実行でき、Native / native lowering 系 WASM は同じ入力を `[E0700]` で拒否します。テストスイートはこの例外を allow-list として固定しており、意図せず成功・失敗の分類が変わった場合は CI で検出されます。
