...
| `taida_abi_web_handle(ptr, len)` | handler を実行し、response handle を返す。 |
| `taida_abi_web_out_ptr(handle)` | response JSON の先頭 pointer。 |
| `taida_abi_web_out_len(handle)` | response JSON の byte length。 |
| `taida_abi_web_free(handle)` | response handle の解放 hook。 |
