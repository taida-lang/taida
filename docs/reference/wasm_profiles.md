...
| `taida-lang/crypto` | ハッシュ系 (`sha256` / `sha512` / `sha384` / `sha224`) / `hmacSha256` / `constantTimeEquals` / `hexEncode` / `base64Encode` | 左記 + `hexDecode` / `base64Decode` / `randomBytes` | ハッシュ系 / `hmacSha256` / `constantTimeEquals` / `hexEncode` / `base64Encode` | `wasm-wasi` と同一 (全 API) |
...
`hexDecode` / `base64Decode` (戻り値 `Lax[Bytes]`) と `randomBytes`
(戻り値 `Bytes`) は Bytes を生成するため、Bytes ランタイムを持つ
`wasm-wasi` / `wasm-full` でのみ利用できます。`randomBytes` は加えて
WASI の `random_get` インポートで OS エントロピーを取得します。
`wasm-min` / `wasm-edge` ではこれら 3 つはコンパイル時に決定的な
「does not support」エラーで reject されます (`wasm-edge` の host
capability 経由の乱数取得は将来の課題)。署名検証 (ed25519 等) は
