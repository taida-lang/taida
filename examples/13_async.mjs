...
function Foldr(list, init, fn) {
  const items = (list && list.__type === 'Stream') ? __taida_stream_collect(list) : list;
  return items.reduceRight(fn, init);
}
...
const resolved = Async(42);
__taida_stdout(("Resolved async: " + resolved.toString()));
const value = await __taida_unmold_async(resolved);
__taida_stdout(("Unwrapped value: " + value.toString()));
__taida_stdout(("isFulfilled: " + resolved.isFulfilled().toString()));
__taida_stdout(("isRejected: " + resolved.isRejected().toString()));
const rejected = AsyncReject("something went wrong");
__taida_stdout(("Rejected async: " + rejected.toString()));
__taida_stdout(("isRejected: " + rejected.isRejected().toString()));
__taida_stdout(("getOrDefault fulfilled: " + resolved.getOrDefault(0).toString()));
__taida_stdout(("getOrDefault rejected: " + rejected.getOrDefault(99).toString()));
function double(x) {
  return (x * 2);
}
...
__taida_stdout(("Mapped (42 * 2): " + doubledVal.toString()));
const rejectedMapped = rejected.map(double);
__taida_stdout(("Map on rejected still rejected: " + rejectedMapped.isRejected().toString()));
const asyncs = Object.freeze([Async(1), Async(2), Async(3)]);
const allResults = await __taida_unmold_async(All(asyncs));
__taida_stdout("All results:");
__taida_stdout(allResults);
const asyncs2 = Object.freeze([Async(10), Async(20)]);
const winner = await __taida_unmold_async(Race(asyncs2));
__taida_stdout(("Race winner: " + winner.toString()));
const timeoutVal = await __taida_unmold_async(Timeout(Async(42), 5000));
__taida_stdout(("Timeout value: " + timeoutVal.toString()));
async function fetchData(key) {
  try {
    const a = AsyncReject("network error");
    const data = await __taida_unmold_async(a);
    return data;
  } catch (error) {
    return ("fallback: " + key);
  }
...
__taida_stdout(("Error ceiling result: " + result));
function addTen(x) {
  return Async((x + 10));
}
...
__taida_stdout(("Chained result (0 + 10 + 10): " + v3.toString()));
