//! C18-2: jsonEncode variant-name Str parity test.
//!
//! Runs `examples/quality/enum_json_encode.td` through all three backends
//! (Interpreter, JS, Native) and asserts byte-identical stdout against the
//! canonical `examples/quality/enum_json_encode.expected`. The expected
//! file fixes the C18-2 wire contract: every Enum value serializes to its
//! declared variant-name Str (e.g. `"Running"`), symmetric with the C16
//! decode direction.
...
fn td_path() -> PathBuf {
    manifest_dir().join("examples/quality/enum_json_encode.td")
}

fn expected_path() -> PathBuf {
    manifest_dir().join("examples/quality/enum_json_encode.expected")
}

fn read_expected() -> String {
    fs::read_to_string(expected_path())
        .expect("examples/quality/enum_json_encode.expected must exist")
}
...
#[test]
fn c18_2_enum_json_encode_interpreter_matches_expected() {
    ...
    assert!(
        ...
        "C18-2 interpreter output mismatch.\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
...
#[test]
fn c18_2_enum_json_encode_js_matches_interpreter() {
    if !node_available() {
    ...
    assert!(
        outputs_equal(&stdout, &expected),
        "C18-2 JS output mismatch (interpreter is reference).\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
...
#[test]
fn c18_2_enum_json_encode_native_matches_interpreter() {
    if !cc_available() {
    ...
    assert!(
        outputs_equal(&stdout, &expected),
        "C18-2 native output mismatch (interpreter is reference).\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
