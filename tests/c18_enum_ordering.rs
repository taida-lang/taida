//! C18-4: Enum same-type ordering comparisons (>=, <, >) parity test.
...
fn td_path() -> PathBuf {
    manifest_dir().join("examples/quality/enum_ordering.td")
}

fn expected_path() -> PathBuf {
    manifest_dir().join("examples/quality/enum_ordering.expected")
}
...
#[test]
fn c18_4_enum_ordering_interpreter_matches_expected() {
    ...
    assert!(
        ...
        "C18-4 interpreter output mismatch.\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
...
#[test]
fn c18_4_enum_ordering_js_matches_interpreter() {
    if !node_available() {
    ...
    assert!(
        outputs_equal(&stdout, &expected),
        "C18-4 JS output mismatch (interpreter is reference).\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
...
#[test]
fn c18_4_enum_ordering_native_matches_interpreter() {
    if !cc_available() {
    ...
    assert!(
        outputs_equal(&stdout, &expected),
        "C18-4 native output mismatch (interpreter is reference).\n--- expected ---\n{}\n--- got ---\n{}\n",
        expected,
        stdout
    );
}
