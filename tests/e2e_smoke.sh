...
for graph_type in dataflow module type-hierarchy error call; do
  graph_output=$($TAIDA graph --type "$graph_type" "$PROJECT_DIR/examples/04_functions.td" 2>/dev/null)
  if [ $? -eq 0 ] && [ -n "$graph_output" ]; then
    pass "graph --type $graph_type"
  else
    fail "graph --type $graph_type"
  fi
done

# Test graph summary
summary_output=$($TAIDA graph summary "$PROJECT_DIR/examples/04_functions.td" 2>/dev/null)
if echo "$summary_output" | grep -q "functions"; then
  pass "graph summary"
else
  fail "graph summary" "$summary_output"
fi

# Test graph --format mermaid
mermaid_output=$($TAIDA graph --format mermaid "$PROJECT_DIR/examples/04_functions.td" 2>/dev/null)
if echo "$mermaid_output" | grep -q "graph LR"; then
  pass "graph --format mermaid"
else
  fail "graph --format mermaid" "$mermaid_output"
fi

# Test graph --format json
json_graph=$($TAIDA graph --format json "$PROJECT_DIR/examples/04_functions.td" 2>/dev/null)
if echo "$json_graph" | grep -q '"view"'; then
  pass "graph --format json"
else
  fail "graph --format json" "$json_graph"
fi
