...
fn run_four_backends(main_path: &Path, dir: &Path) -> [(String, String); 4] {
    assert!(
        node_available(),
        "node is required for 4-backend E38 file Read parity"
    );
    assert!(
        cc_available(),
        "cc is required for 4-backend E38 file Read parity"
    );
    assert!(
        wasmtime_available(),
        "wasmtime is required for 4-backend E38 file Read parity"
    );
...
#[test]
fn read_missing_file_carries_error_info_across_backends() {
    ...
    fs::write(
        ...
        concat!(
            ">>> taida-lang/os => @(Read)\n",
            "bad <= Read[\"__taida_e38_missing_read_errorinfo__\"]()\n",
            "info <= bad.errorInfo()\n",
