...
fn fixture_dir(tag: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "e38_http_request_error_info_{}_{}_{}",
        tag,
        std::process::id(),
        nanos
    ));
...
fn run_three_backends(main_path: &Path, dir: &Path) -> [(String, String); 3] {
    assert!(
        node_available(),
        "node is required for 3-backend E38 HttpRequest parity"
    );
    assert!(
        cc_available(),
        "cc is required for 3-backend E38 HttpRequest parity"
    );
...
#[test]
fn http_request_invalid_scheme_carries_error_info_across_backends() {
    let dir = fixture_dir("invalid_scheme");
