...
fn fixture_dir(tag: &str) -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!(
        "e38_read_bytes_at_error_info_{}_{}_{}",
        tag,
        std::process::id(),
        nanos
    ));
...
fn run_four_backends(main_path: &Path, dir: &Path) -> [(String, String); 4] {
    assert!(
        node_available(),
        "node is required for 4-backend E38 readBytesAt parity"
    );
    assert!(
        cc_available(),
        "cc is required for 4-backend E38 readBytesAt parity"
    );
    assert!(
        wasmtime_available(),
        "wasmtime is required for 4-backend E38 readBytesAt parity"
    );
...
#[test]
fn read_bytes_at_missing_file_carries_error_info_across_backends() {
    ...
    fs::write(
        ...
        concat!(
            ...
            "bad <= readBytesAt(\"__taida_e38_missing_read_bytes_at_errorinfo__\", 0, 4)\n",
            "info <= bad.errorInfo()\n",
