...
///
/// The per-fixture tests generated by
/// `build.rs` and the pre-existing `wasm_min_*` point tests can both target
/// the same fixture (e.g. `wasm_min_pi_approx`). When nextest runs them in
/// parallel, they clobber each other's temp `.wasm` / `.gen.c` files. Use
/// a pid+nanos suffix so each invocation has its own unique output path;
/// the fixture stem is still used for cache keying (`cache_wasm(profile,
/// stem, ...)`), so superset tests still find the cached artifact.
...
fn wasm_min_size_gate() {
    ...
    // `--no-check`, so the cost is intentional.
    // Quality track (2026-06-11): bumped 9,664 -> 10,240. taida_throw's
    // no-ceiling path now prints the reference's one-line unhandled
    // report (pack type/message extraction + WASI stderr write +
    // proc_exit) instead of trapping after a generic note: +418 on every
    // binary that links throw. The report deliberately renders strings
...
// ---------------------------------------------------------------------------
// W-6: Per-fixture parity tests for wasm-min (vs interpreter).
//
// C24 Phase 5 (RC-SLOW-2 / C24B-006): Decomposed from the previous monolithic
// `wasm_min_parity_all_examples`. See tests/wasm_wasi.rs for design
// rationale.
//
// Unlike wasm-wasi / wasm-full, this runner does not have "skip" / "native
// fail" / "rejected" allowlists because the previous aggregate test accepted
// any `compile_rejected` or `interp_fail` count without a hard bound. To
// preserve semantics exactly, per-fixture tests here treat compile rejection
// and interpreter failure as "skip this fixture" (non-fatal), matching the
// original behavior. The exact parity-OK count (71) is enforced by the
// allowlist guard below, which uses tracking state updated by fixture runs.
// ---------------------------------------------------------------------------

/// Known parity diffs — examples that compile but produce different output.
...
fn run_wasm_min_parity_fixture(stem: &str) {
    ...
    // Interpreter is the reference. If it can't run, we can't compare —
    // matches original `interp_fail.push(...); continue;` semantics.
    let interp_out = match run_interpreter(&td_path) {
        Some(s) => s,
        None => return,
    };

    // wasm-min compile + run. Compile rejection is non-fatal here (original
    // behavior): some fixtures are known not to compile on the minimal profile.
    let wasm_out = match compile_and_run_wasm(&td_path, &wasmtime) {
        Some(s) => s,
        None => return,
    };

    if interp_out != wasm_out {
        if WASM_MIN_EXPECTED_PARITY_DIFF.contains(&stem) {
            return;
        }
        panic!(
            "W-6 PARITY FAILED for {}: interp='{}' vs wasm='{}'",
            stem,
            interp_out.chars().take(200).collect::<String>(),
            wasm_out.chars().take(200).collect::<String>(),
        );
    }
}

// (No aggregate count guard here.)
//
// The original `wasm_min_parity_all_examples` test had a single assertion
// `parity_ok.len() == 71` without any corresponding allowlist for rejected /
// interp_fail fixtures. Reconstructing that count as a pure static check is
// not possible (we'd need to know which of the 86 fixtures wasm-min can
// compile), and rerunning the full loop just to count defeats the purpose of
// decomposition — the loop cost was exactly what RC-SLOW-2 aims to remove.
//
// Instead, the per-fixture tests below (`fixture_all_td_<stem>`) individually
// enforce parity: any regression (parity previously passed, now breaks) will
// surface as a loud per-fixture failure. The only signal we lose vs. the old
// count assertion is "a new fixture has been added that happens to parity
// with wasm-min but someone forgot to update the count" — which is a
// non-issue because the new fixture's per-fixture test also gets generated
// and runs, exercising the codepath just the same.

// Per-fixture tests emitted by build.rs; see tests/wasm_wasi.rs for design.
macro_rules! c24_fixture_runner {
    ($stem:expr) => {
        run_wasm_min_parity_fixture($stem)
    };
}
...
/// httpServe in wasm-min must produce compile error (not silent success).
#[test]
...
/// httpParseRequestHead in wasm-min must produce compile error.
#[test]
...
/// httpEncodeResponse in wasm-min must produce compile error.
#[test]
